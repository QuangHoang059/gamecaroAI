...
class Labal:
    ...
    def drawlb(self):
        ...
        pg.draw.rect(self.screen, self.bg, self.lbRect, border_radius=self.borderradix)
...
class Button:
    ...
    def draw(self):
        ...
        pg.draw.rect(self.screen, self.bottom_color, self.bottom_rect, border_radius=12)
...
class Game1x1:
    def __init__(self, screen):
        ...
        self.imgs = []
        self.imgs.append(
            pg.transform.scale(pg.image.load(".\img\symbol-check-icon.png"), (31, 31))
        )
        self.imgs.append(
            pg.transform.scale(pg.image.load(".\img\delete-icon.png"), (31, 31))
        )
        self.player = 1
        self.cooldown = 0
    ...
    def __coolDown(self):
        ...
        if width < 0:
            ...
            self.map[self.checkpoint[0]][self.checkpoint[1]] = self.player
            self.cooldown = 0
    ...
    def playerisclikMouse(self):
        ...
        if 0 <= x_mouse < self.lenmap[0] and 0 <= y_mouse < self.lenmap[1]:
            if (
                self.map[y_mouse // 31][x_mouse // 31] == 0
                and self.event.type == pg.MOUSEBUTTONDOWN
            ):
    ...
    def __checkwin(self):
        ...
        if player != 0:
            x = i
            # kiểm tra cột
            while (x > 0) and (self.map[x - 1][j] == player):
                coutcolum = coutcolum + 1
                x = x - 1
            tail = [x - 1, j]
            x = i
            while (x < lenx) and (self.map[x + 1][j] == player):
                coutcolum = coutcolum + 1
                x = x + 1
            head = [x + 1, j]
            ...
            if coutcolum == 5:
                if not (
                    (tail[0] == -1 and self.map[head[0]][head[1]] == 3 - player)
                    or (
                        head[0] == lenx + 1 and self.map[tail[0]][tail[1]] == 3 - player
                    )
                    or (
                        tail[0] >= 0
                        and head[0] < lenx + 1
                        and self.map[tail[0]][tail[1]] == 3 - player
                        and self.map[head[0]][head[1]] == 3 - player
                    )
                ):
                    if player == 1:
                        self.playerwin = 1
            ...
            # kiểm tra hàng
            y = j
            while (y > 0) and (self.map[i][y - 1] == player):
                countrow = countrow + 1
                y = y - 1
            tail = [i, y - 1]
            y = j
            while (y < leny) and (self.map[i][y + 1] == player):
                countrow = countrow + 1
                y = y + 1
            head = [i, y + 1]
            ...
            if countrow == 5:
                if not (
                    (tail[1] == -1 and self.map[head[0]][head[1]] == 3 - player)
                    or (
                        head[1] == leny + 1 and self.map[tail[0]][tail[1]] == 3 - player
                    )
                    or (
                        tail[1] >= 0
                        and head[1] < leny + 1
            ...
            y = j
            x = i
            while ((y > 0) and (x > 0)) and (self.map[x - 1][y - 1] == player):
            ...
            y = j
            x = i
            while ((y < leny) and (x < lenx)) and (self.map[x + 1][y + 1] == player):
                coutcrossleft = coutcrossleft + 1
                y = y + 1
                x = x + 1
            head = [x + 1, y + 1]
            ...
            if coutcrossleft == 5:
                if not (
                    (
                        (tail[0] == -1 or tail[1] == -1)
                        and self.map[head[0]][head[1]] == 3 - player
                    )
                    or (
                        (head[0] == lenx + 1 or head[1] == leny + 1)
                        and self.map[tail[0]][tail[1]] == 3 - player
                    )
                    or (
                        tail[0] >= 0
            ...
            y = j
            x = i
            while ((y < leny) and (x > 0)) and (self.map[x - 1][y + 1] == player):
            ...
            y = j
            x = i
            while ((y > 0) and (x < lenx)) and (self.map[x + 1][y - 1] == player):
                coutcrossright = coutcrossright + 1
                y = y - 1
                x = x + 1
            head = [x + 1, y - 1]
            if coutcrossright == 5:
                if not (
                    (
                        (tail[0] == -1 or tail[1] == leny + 1)
                        and self.map[head[0]][head[1]] == 3 - player
                    )
                    or (
                        (head[0] == lenx + 1 or head[1] == -1)
                        and self.map[tail[0]][tail[1]] == 3 - player
                    )
                    or (
                        tail[0] >= 0
    ...
    def draw(self):
        x = self.lenmap[0]
        y = self.lenmap[0]
        pg.draw.rect(self.screen, "white", (0, 0, x, y))
        x_mouse, y_mouse = pg.mouse.get_pos()
        if 0 <= x_mouse < self.lenmap[0] and 0 <= y_mouse < self.lenmap[1]:
            pg.draw.rect(
                self.screen,
        ...
        for i in range(0, x + 31, 31):
            ...
            for j in range(0, y + 31, 31):
                pg.draw.line(self.screen, "black", [0, j], [y, j])  # draw line col
                if i < x and j < y:
    ...
    def __drawWinLoss(self):
        if self.playerwin != 0:
            ...
            imgs.append(
                pg.transform.scale(
                    pg.image.load(
                        ".\img\Lovepik_com-401644461-win-victory-champion-word-design.png"
                    ),
                    (500, 500),
                )
            )
            imgs.append(
                pg.transform.scale(pg.image.load(".\img\pngegg.png"), (500, 500))
            )
            if self.playerwin == 1:
